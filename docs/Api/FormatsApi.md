# EtkinlikIo\Api\FormatsApi

Operations related to event formats (types).
...
$apiInstance = new EtkinlikIo\Api\Api\FormatsApi(
...
try {
    $result = $apiInstance->listFormats();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling FormatsApi->listFormats: ', $e->getMessage(), PHP_EOL;
}
```
